...
import { Iso15693Memory_FieldsDefinition } from './iso-15693-memory-fields-definition';
import { Iso_15693_memory_getFieldsList } from './iso-15693-memory-fields-list';
import { Iso_15693_memoryFormGroup } from './iso-15693-memory-form-group';

@Component({
  selector: 'app-iso-15693-memory',
  templateUrl: './iso-15693-memory.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class Iso15693MemoryComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'Iso15693MemoryComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - ISO 15693 Memory Template');
    // prettier-ignore
    this.createConfigurationObjects(Iso_15693_memoryFormGroup, Iso15693Memory_FieldsDefinition, Iso_15693_memory_getFieldsList);   }
