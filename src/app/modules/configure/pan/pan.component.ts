...
import { Pan_FieldsDefinition } from './pan-fields-definition';
import { Pan_getFieldsList } from './pan-fields-list';
import { PanFormGroup } from './pan-form-group';
...
export class PanComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'PanComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - PAN Template');
    // prettier-ignore
    this.createConfigurationObjects(PanFormGroup, Pan_FieldsDefinition, Pan_getFieldsList);   }
