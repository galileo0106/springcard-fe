...
import { DesfireId_FieldsDefinition } from './desfire-id-fields-definition';
import { Desfire_id_getFieldsList } from './desfire-id-fields-list';
import { Desfire_idFormGroup } from './desfire-id-form-group';

@Component({
  selector: 'app-desfire-id',
  templateUrl: './desfire-id.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class DesfireIdComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'DesfireIdComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Desfire ID Template');
    // prettier-ignore
    this.createConfigurationObjects(Desfire_idFormGroup, DesfireId_FieldsDefinition, Desfire_id_getFieldsList);   }
