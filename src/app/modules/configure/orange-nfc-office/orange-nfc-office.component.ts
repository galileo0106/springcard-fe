...
import { OrangeNfcOffice_FieldsDefinition } from './orange-nfc-office-fields-definition';
import { Orange_nfc_office_getFieldsList } from './orange-nfc-office-fields-list';
import { Orange_nfc_officeFormGroup } from './orange-nfc-office-form-group';

@Component({
  selector: 'app-orange-nfc-office',
  templateUrl: './orange-nfc-office.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class OrangeNfcOfficeComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'OrangeNfcOfficeComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Orange NFC Office Template');
    // prettier-ignore
    this.createConfigurationObjects(Orange_nfc_officeFormGroup, OrangeNfcOffice_FieldsDefinition, Orange_nfc_office_getFieldsList);   }
