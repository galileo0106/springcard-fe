...
import { AppleVas_FieldsDefinition } from './apple-vas-fields-definition';
import { Apple_vas_getFieldsList } from './apple-vas-fields-list';
import { Apple_vasFormGroup } from './apple-vas-form-group';

@Component({
  selector: 'app-apple-vas',
  templateUrl: './apple-vas.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class AppleVasComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'AppleVasComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Apple VAS Template');
    // prettier-ignore
    this.createConfigurationObjects(Apple_vasFormGroup, AppleVas_FieldsDefinition, Apple_vas_getFieldsList);   }
