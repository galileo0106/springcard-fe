...
import { GoogleSmarttap_FieldsDefinition } from './google-smarttap-fields-definition';
import { Google_smarttap_getFieldsList } from './google-smarttap-fields-list';
import { Google_smarttapFormGroup } from './google-smarttap-form-group';

@Component({
  selector: 'app-google-smarttap',
  templateUrl: './google-smarttap.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class GoogleSmarttapComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'GoogleSmarttapComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Google SmartTap Template');
    // prettier-ignore
    this.createConfigurationObjects(Google_smarttapFormGroup, GoogleSmarttap_FieldsDefinition, Google_smarttap_getFieldsList);   }
