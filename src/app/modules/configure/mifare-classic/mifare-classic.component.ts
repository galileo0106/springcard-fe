...
import { MifareClassic_FieldsDefinition } from './mifare-classic-fields-definition';
import { Mifare_classic_getFieldsList } from './mifare-classic-fields-list';
import { Mifare_classicFormGroup } from './mifare-classic-form-group';

@Component({
  selector: 'app-mifare-classic',
  templateUrl: './mifare-classic.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class MifareClassicComponent extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'MifareClassicComponent', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Mifare Classic Template');
    // prettier-ignore
    this.createConfigurationObjects(Mifare_classicFormGroup, MifareClassic_FieldsDefinition, Mifare_classic_getFieldsList);   }
