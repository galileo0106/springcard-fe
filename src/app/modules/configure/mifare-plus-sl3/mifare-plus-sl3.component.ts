...
import { MifarePlusSl3_FieldsDefinition } from './mifare-plus-sl3-fields-definition';
import { Mifare_plus_sl3_getFieldsList } from './mifare-plus-sl3-fields-list';
import { Mifare_plus_sl3FormGroup } from './mifare-plus-sl3-form-group';

@Component({
  selector: 'app-mifare-plus-sl3',
  templateUrl: './mifare-plus-sl3.component.html',
  styleUrls: ['../sharedstyles.css'],
})
export class MifarePlusSl3Component extends TemplateComponent implements OnInit, OnDestroy {
  @Input() configId = 0;
  @Input() deviceId = '';
  @Input() UUID = '';
  ...
  ngOnInit() {
    ...
    this.setInputParameters(this.configId, this.deviceId, this.UUID, this.componentName, this.deviceList, 'MifarePlusSl3Component', 'nfc', this.slot, this.configurationIsInReadOnlyMode);
    this.currentTitleService.changeTitle('Configuration - Mifare Plus SL3 Template');
    // prettier-ignore
    this.createConfigurationObjects(Mifare_plus_sl3FormGroup, MifarePlusSl3_FieldsDefinition, Mifare_plus_sl3_getFieldsList);   }
