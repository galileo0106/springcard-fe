...
export class LoginComponent implements OnInit {
  ...
  googleOAuthUrl = environment.googleOAuthUrl;
  ...
  selectConnectionMethod() {
    if (this.authService.hasPreviousLoginInformation() && this.authService.previousConnectionWasMadeWithSocialProvider()) {
      this.loginWithSocialProvider('google');
    }
  }
  ...
  logout() {
    ...
    this.snackBar.open($localize`You are disconnected`, $localize`Disconnect`, {
      duration: 4000,
    });
    this.isConnected = this.authService.isLoggedIn();
  }
  ...
  loginWithSocialProvider(socialProvider: string) {
    this.authService.loginWithSocialProvider(socialProvider).subscribe(
